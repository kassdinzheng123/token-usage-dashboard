...
pub fn load_source_view(view: &str, _refresh: bool) -> Result<Vec<Value>, String> {
    let sessions = load_sessions().map_err(|err| err.to_string())?;
    Ok(match view {
...
pub fn load_sessions() -> Result<Vec<LocalSession>, SourceError> {
    ...
    for agent_entry in agent_entries {
        ...
        append_sessions_from_value(&mut sessions, &data, &sessions_dir);
    }

    Ok(sessions)
}

fn append_sessions_from_value(sessions: &mut Vec<LocalSession>, data: &Value, sessions_dir: &Path) {
...
fn session_from_message_event(
    ...
    let input_tokens = usage_i64(usage.get("input").or_else(|| usage.get("inputTokens")));
    ...
    let total_tokens_override = usage
        .get("totalTokens")
        .or_else(|| usage.get("total_tokens"))
        .map(|value| usage_i64(Some(value)))
...
fn session_from_summary(key: &str, entry: &serde_json::Map<String, Value>) -> Option<LocalSession> {
    ...
    let input_tokens = summary_i64(entry, &["inputTokens", "input"]);
    ...
    let total_tokens_override = entry
        .get("totalTokens")
        .or_else(|| entry.get("total_tokens"))
        .map(|value| usage_i64(Some(value)))
        .filter(|tokens| *tokens > 0);
...
"#,
        ...
        with_home(&fixture.path, || {
            ...
            assert_eq!(daily[0]["inputTokens"], 10);
            assert_eq!(daily[0]["totalTokens"], 19);
            assert_eq!(daily[0]["totalCost"], 0.25);
            assert_eq!(daily[1]["date"], "2024-02-15");
            assert_eq!(daily[1]["inputTokens"], 5);
            assert_eq!(daily[1]["totalTokens"], 8);

            let monthly = load_source_view("monthly", false).unwrap();
            assert_eq!(monthly.len(), 2);
            assert_eq!(monthly[0]["month"], "2024-01");
            assert_eq!(monthly[0]["totalTokens"], 19);
            assert_eq!(monthly[1]["month"], "2024-02");
            assert_eq!(monthly[1]["totalTokens"], 8);
        });
    }
